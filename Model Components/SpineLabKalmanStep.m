...
%     weights = [0.1,1,1];
    ...
    weights = [[100, 100, 100]*fss_mat*inv(fss_mat+sig_t_t1); [1,1,1]*sig_t_t1*inv(fss_mat+sig_t_t1)];
    ...
    sig_t_t = combined_sig; %combined_sig;
    % Fusion
...
function [x_t_t, sig_t_t] = SpineLabKalmanStep2( x_t1_t1, sig_t1_t1, alpha, params, dt, tnow )
    ...
    [fs, fss, canal_weights] = CanalParticleWeighting( [canal1_est, canal2_est], [0,0,0], [diag( sigPrior )'] );
    new_canal_est = fs;
    new_canal_sig = sig_sensor + canal_weights(1)*diag( canal1_est.*canal1_est ) + canal_weights(2)*diag( canal2_est.*canal2_est ) - diag( new_canal_est.*new_canal_est );
