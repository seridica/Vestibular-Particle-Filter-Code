    ...
    for i=1:size( all_means, 2 )
        ...
        for j=1:size( all_means, 1 )
            if sum(all_sigs(j,i) == 0) == 0
                afferent_weights = afferent_weights .* normpdf( particle_afferents(i,:)', all_means(j,i), sqrt( all_sigs(j,i) ) );
            end
            % Renormalize each time to prevent weights going to zero
            if sum( afferent_weights ) < 1e-16
                if j == 1
                    afferent_weights = ones( nAfferents, 1 ) / nAfferents;
                else
                    sum( afferent_weights )
                    afferent_weights = zeros( nAfferents, 1 );
                    [m, pind] = min( ( particle_afferents(i,:) - all_means(j,i) ).^2 );
                    afferent_weights(pind) = 1;
                    %afferent_weights = ones( nAfferents, 1 ) / nAfferents;
                end
            else
        ...
        % Weighted mean
        fused_estimate(i) = ( particle_afferents(i,:) * afferent_weights ); % / nAfferents;
        fused_sigma(i) = 0.2; %max( [( (particle_afferents(i,:) - fused_estimate(i) ).^2 * afferent_weights ), 0.2] );
        %fused_sigma(i) = (particle_afferents(i,:) - fused_estimate(i) ).^2 * afferent_weights;
    end
end